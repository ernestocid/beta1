...
/**
 * This class is responsible for gathering machine information for test case generation.
 * These information are: input space variables and characteristics of this variables.
 * 
 * @author ernestocid
 *
 */
public class Partitioner {
	...
	/**
	 * This method creates a set containing all input space variables
	 ...
	 * Invariant clauses. 
	 * 
	 * @param operation under test
	 * @return set of input space variables present on precondition, 
	...
	private Set<String> getRelatedVariables(Operation operation) {
		...
		if(this.operation.getPrecondition() != null) {
			...
			// Search for more related variables on the invariant and add them to the
			...
			// Remove constants from enumerated sets that might be added in the previous steps
	...
	/**
	 * Searches a machine's invariant looking for related variables. It also searches on
	 * invariants of other machines included. Every time the set of related variables is updated
	 * a new search is done to check for variables that are related to the ones just added.
	 * The method ends when no related variables are found after the last search.
	 *   
	 * @param relatedVariables initial set of related variables
	 ...
	 *  
	 * @param relatedVariables set of related variables
	 */
	 ...
	 * Those are constant values from enumerated sets that might be mistaken as variables.
	 * 
	 * @param relatedVariables set of related variables
	 */
	...
	private void removeConstants(Set<String> relatedVariables) {
		if(this.operation.getMachine().getIncludes() != null) {
			...
			for(Machine includedMachine : machinesIncluded) {
				if(includedMachine.getConstants() != null) {
					relatedVariables.removeAll(includedMachine.getConstants().getAll());
				}
			}
		}
	...
	protected Set<Characteristic> getGeneralCharacteristicsClauses() {
		...
		// The order from where you get the characteristics is important here if there are
	...
	// It is not checking the type of the characteristic. So there might occur that clauses telling 
	...
	private Set<Characteristic> getCharacteristicsFromPrecondition() {
	...
	/**
	 * Creates a set of predicates that contain related variables. Receives a set of
	 * related variables and a set of predicates and creates a new set containing all the
	 * predicates that mention at least one related variable.
	 * 
	 * @param variables set of related variables that are going to be searched
	 * @param predicates set of predicates that are going to be investigated
