...
public class PartitionerTest {
	...
	@Test
	public void shouldGetInputSpaceWhenInputSpaceVariablesAreUsedOnIntervalExpressions() {
		Machine machine = getMachineInstance("src/test/resources/machines/LuaSpec/src/TypeCheckOperations.mch");
		Operation operationUnderTest = machine.getOperation(0);

		Partitioner partitioner = new Partitioner(operationUnderTest);

		Set<String> expectedInputSpace = new HashSet<String>();
		expectedInputSpace.add("index");
		expectedInputSpace.add("max_stack_top");
		expectedInputSpace.add("stack_top");
	...
	public void shouldGetOperationCharacteristics2() {
		...
		expectedCharacteristics
				.add("!(abonne,ex1,ex2).((abonne : Abonnes & ex1 : Exemplaires & ex2 : Exemplaires & ex1 /= ex2 & ex1 : dom((est_emprunte_par |> {abonne})) & ex2 : dom((est_emprunte_par |> {abonne}))) => (est_exemplaire_de(ex1) /= est_exemplaire_de(ex2)))");
